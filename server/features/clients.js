...
rutas.route('/')
  .get(function (req, res) {
    ...
    if (client.id) {
      ...
      dataParams.push(client.id);
    }

    if (client.legacy_id) {
      commonQuery += 'AND LEGACY_ID IN (?) ';
      dataParams.push(client.legacy_id);
    }

    if (client.description) {
      commonQuery += 'AND DESCRIPTION LIKE ? ';
      dataParams.push('%' + client.description + '%');
    }
  ...
  .post(function (req, res) {
    ...
    client.legacy_id = clientBP.legacy_id;
    ...
    db.query('INSERT INTO CLIENTS SET ?;', req.body, function (err, result) {
      if (err) throw err;
